...
function fn_rma_update_details($data)
{
    ...
    if ((!$show_confirmation || ($show_confirmation && YesNo::toBool($confirmed))) && $change_return_status['status_from'] != $change_return_status['status_to']) {
        ...
        if (Registry::get('settings.Checkout.tax_calculation') === 'unit_price' && isset($order_info['subtotal'], $order_info['taxes'])) {
            $product_tax_subtotal = 0.0;
            foreach ($order_info['taxes'] as $tax_data) {
                if (empty($tax_data['applies'])) {
                    continue;
                }
                if (YesNo::toBool($tax_data['price_includes_tax'])) {
                    continue;
                }
                foreach ($tax_data['applies'] as $tax_code => $tax_value) {
                    //phpcs:ignore
                    if (strpos($tax_code, 'P_') === 0) {
                        $product_tax_subtotal += $tax_value;
                    }
                }
            }
            $order_info['subtotal'] -= $product_tax_subtotal;
        }
        ...
        if (
            ...
            $old_shipping_cost = $old_order_info['shipping_cost'];
            if (
                Registry::get('settings.Checkout.tax_calculation') === 'unit_price'
                && isset($old_order_info['taxes'])
                && is_array($old_order_info['taxes'])
            ) {
                foreach ($old_order_info['taxes'] as $tax) {
                    if (empty($tax['applies'])) {
                        continue;
                    }
                    if (YesNo::toBool($tax['price_includes_tax'])) {
                        continue;
                    }
                    foreach ($tax['applies'] as $tax_id => $tax_value) {
                        if (strpos($tax_id, 'S_') !== 0) {
                            continue;
                        }
                        $old_shipping_cost -= $tax_value;
                    }
                }
            }
            $payout_data['order_amount'] = ($payout_amount + $taxes_amount + $old_shipping_cost - $payout_shipping_cost) * $payout_amount_sign;
