...
function fn_gift_certificates_rma_update_details_create_payout(array $data, array $order_info, array $return_info, array &$payout_data)
{
    ...
    foreach ($order_info['use_gift_certificates'] as $gift_certificate) {
        ...
        if ($payout_data['order_amount'] !== 0.0) {
            $coefficient = round(abs($gift_certificate['cost'] / $payout_data['order_amount']), 2);
        } else {
            $coefficient = 0;
        }
        ...
        if ($is_refund) {
            $payout_data['marketplace_profit'] = round($coefficient * $payout_data['marketplace_profit'], 2);
            $payout_data['commission_amount'] = round($coefficient * $payout_data['commission_amount'], 2);
            continue;
        }
    }
}
