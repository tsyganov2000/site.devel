    ...
    if ($type == 'to_fixed' || $type == 'by_fixed') {
        ...
        if (empty($cart['parent_order_id'])) {
            ...
            if (!empty($parent_order['subtotal'])) {
                ...
                if ($tax_calculation_type === 'unit_price') {
                    $product_taxes = 0.0;
                    if (is_array($parent_order['taxes'])) {
                        foreach ($parent_order['taxes'] as $tax_data) {
                            ...
                            foreach ($tax_data['applies'] as $tax_code => $tax_value) {
                                ...
                                if (strpos($tax_code, 'P_') === 0) {
                                    $product_taxes += $tax_value;
                                }
                            }
                        }
                    }
                    $parent_order['subtotal'] -= $product_taxes;
                }
