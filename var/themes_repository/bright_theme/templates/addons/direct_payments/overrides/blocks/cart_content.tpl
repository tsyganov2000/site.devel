...
{hook name="checkout:cart_content"}
    <div class="ty-dropdown-box" id="cart_status_{$dropdown_id}">
        ...
        <a href="{"checkout.cart"|fn_url}">
            ...
            {$additional_class = ($cart.amount) ? "filled" : "empty"}
            {include_ext file="common/icon.tpl"
                class="ty-icon-moon-commerce ty-minicart__icon `$additional_class`"
            }
            <span class="ty-minicart-title{if !$cart.amount} empty-cart{/if} ty-hand">
                <span class="ty-block ty-minicart-title__header ty-uppercase">{__("my_cart")}</span>
                <span class="ty-block">
                {if $cart.amount}
                    {$cart.amount}&nbsp;{__("items")} {__("for")}&nbsp;{include file="common/price.tpl" value=$cart.display_subtotal}
                {else}
                    {__("cart_is_empty")}
                {/if}
                </span>
                            ...
                            {if $cart.amount}
                                <ul class="ty-cart-items__list">
                                    {hook name="index:cart_status"}
                                        {assign var="_cart_products" value=$cart.products}
