...
<div class="table-wrapper category-tree-wrapper">
    ...
    <tr id="{$comb_id}_container"
        class="cm-row-status-{$category.status|lower}{if $has_children} cm-click-on-visible{else} cm-toggle-checked{/if}{if !$cur_cat.storefront_categories} cm-click-and-close{/if} {if $display == "radio"}row-actionable cm-click-and-close-forced{/if}"
        {if $has_children}
    ...
    >
        ...
        <td class="left first-column" width="5%">
            {if $cur_cat.storefront_categories}
                &nbsp;
                {assign var="comb_id" value="comp_`$cur_cat.storefront_id`_`$random`"}
