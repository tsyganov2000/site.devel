...
<div class="control-group profile-field-{$field.field_name}">
    ...
    {if $field.field_type == "ProfileFieldTypes::STATE"|enum}
        ...
        <select class="cm-state {if $section == "S"}cm-location-shipping{else}cm-location-billing{/if} {if !$states.$_country}hidden cm-skip-avail-switch{/if}" id={$element_id} name="{$data_name}[{$data_id}]" {$disabled_param nofilter}>
        ...
        <input type="text" id="elm_{$field.field_id}_d" name="{$data_name}[{$data_id}]" size="32" maxlength="64" value="{$_state}" disabled="disabled" class="cm-state {if $section == "S"}cm-location-shipping{else}cm-location-billing{/if} input-large {if $states.$_country}hidden cm-skip-avail-switch{/if}" />
